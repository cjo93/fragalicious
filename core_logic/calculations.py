        ...
        for i in range(days):
            ...
            if abs(t_sun_lon - natal_chart.sun.longitude) < 1:
                events.append(ForecastEvent(
                    ...
                    title="Solar Core Alignment",
                    description="Primary cycle reset. Maximum kinetic throughput. Calibrate protocols.",
                    intensity=10,
                    type="ALIGNMENT"
                ))
            ...
            if abs(t_moon_lon - natal_chart.sun.longitude) < 6: # Moon moves fast (~12 deg/day), wide orb
                events.append(ForecastEvent(
                    date=check_date.strftime("%Y-%m-%d"),
                    title="Lunar-Solar Logic Sync",
                    description="Emotional data streams align with core directives. Optimization phase.",
                    intensity=7,
                    type="ALIGNMENT"
                ))
